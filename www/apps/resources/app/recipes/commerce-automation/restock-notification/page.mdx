...
Learn more about data models in [this documentation](!docs!/learn/basics/modules#1-create-data-model).
...
Learn more about data model [properties](!docs!/learn/advanced-development/data-models/property-types) and [relations](!docs!/learn/advanced-development/data-models/relationships).
...
Learn more about data model indexes in [this documentation](!docs!/learn/advanced-development/data-models/index).
...
Learn more about services in [this documentation](!docs!/learn/basics/modules#2-create-service).
...
Learn more about migrations in [this documentation](!docs!/learn/basics/modules#5-generate-migrations).
...
Learn more about module links in [this documentation](!docs!/learn/advanced-development/module-links).
...
- Use [transform](!docs!/learn/advanced-development/workflows/variable-manipulation) from the Workflows SDK to create a `customerId` variable. Its value is either the ID of the customer passed in the workflow's input if it's not `undefined`, or an empty string.
- Use [when-then](!docs!/learn/advanced-development/workflows/conditions) from the Workflows SDK that performs steps if a condition is met. If the customer's email isn't set in the workflow's input, you retrieve the customer using `useQueryGraphStep` by its ID.
...
A workflow's constructor function has some constraints in implementation, which is why you need to use `transform` for variable manipulation and `when-then` to perform steps based on a condition. Learn more about these constraints in [this documentation](!docs!/learn/advanced-development/workflows/constructor-constraints).
...
- Use [when-then](!docs!/learn/advanced-development/workflows/conditions) to perform an action if a condition is met.
...
Learn more about API routes in [this documentation](!docs!/learn/basics/api-routes).
...
1. `useQueryGraphStep` to retrieve all subscriptions that have a restocked variant and sales channel ID pairing using [Query](!docs!/learn/advanced-development/module-links/query). Notice that in the specified `fields` you pass `product_variant.*`, which retrieves the details of the subscription's variant from the Product Module. This is possible due to the module link you created between the `RestockSubscription` and `ProductVariant` models in an earlier step.
...
Learn more about scheduled jobs in [this documentation](!docs!/learn/basics/scheduled-jobs).
