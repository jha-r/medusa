...
- `REDIS_URL`'s value is automatically generated using Railway’s template syntax. Please note that Railway does not support IPv4 private networking, so make sure to add the `?family=0` to the connection string to support both IPv6 and IPv4 connections.
